...
export const LOCAL_STORAGE_KEY_PREFRENCES_SELECTED =
  "koliyati.prefrences.selected";
...
export const BASE_URL = "https://kolyati-backend.herokuapp.com/api/v1/";
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const parsedData = JSON.parse(
      localStorage.getItem(LOCAL_STORAGE_KEY_FIRST_TIME)!
    );
  ...
  const changeFirstTimeState = () => {
    localStorage.setItem(
      LOCAL_STORAGE_KEY_FIRST_TIME,
      JSON.stringify({ firstTime: false })
    );
