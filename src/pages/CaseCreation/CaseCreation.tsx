...
import {
  ...
  LOCAL_STORAGE_KEY_GPA,
  LOCAL_STORAGE_KEY_MAJORS,
  LOCAL_STORAGE_KEY_MAJORS_SHOW,
  LOCAL_STORAGE_KEY_TAWIJIHI_TYPE,
  LOCAL_STORAGE_KEY_PREFRENCES,
  LOCAL_STORAGE_KEY_PREFRENCES_SELECTED,
  LOCAL_STORAGE_KEY_DESCRIPTION,
  LOCAL_STORAGE_KEY_LOCATION,
  LOCAL_STORAGE_KEY_NAME,
  LOCAL_STORAGE_KEY_CREATED,
  LOCAL_STORAGE_KEY_CASSES_ID,
} from "../../containers/App";
...
const CaseCreation: React.FC = () => {
  ...
  const [prefrences, setPrefrences] = useState<string[]>([
    "سهولة الدراسة",
    "فرص العمل",
    "المواصلات",
    "التكاليف الدراسية",
  ...
  const [toastMessage, setToastMessage] = useState<string>(
    "لا يمكنك إضافة هذا التخصص، يجب إختيار 3 تخصصات مختلفة فقط"
  );

  const history = useHistory();
  useEffect(() => {
    const tawjihiTypeData = localStorage.getItem(
      LOCAL_STORAGE_KEY_TAWIJIHI_TYPE
    );
    if (tawjihiTypeData) {
      ...
      if (gpaData) {
        ...
        const showMajorsData = JSON.parse(
          localStorage.getItem(LOCAL_STORAGE_KEY_MAJORS_SHOW)!
        );
        const majorsData = JSON.parse(
          localStorage.getItem(LOCAL_STORAGE_KEY_MAJORS)!
        );
        if (majorsData && majorsData.length > 0) setMajorChoice(majorsData);
        if (!showMajorsData) {
          setShowMajors(false);
        }
      }
    ...
    const prefrencesData = JSON.parse(
      localStorage.getItem(LOCAL_STORAGE_KEY_PREFRENCES)!
    );
    if (prefrencesData) setPrefrences(prefrencesData);
    const prefrencesSelectedData = JSON.parse(
      localStorage.getItem(LOCAL_STORAGE_KEY_PREFRENCES_SELECTED)!
    );
    if (prefrencesSelectedData) setPrefenceSelected(prefrencesSelectedData);
    const nameData = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY_NAME)!);
    if (nameData) setName(nameData);
    const locationData = JSON.parse(
      localStorage.getItem(LOCAL_STORAGE_KEY_LOCATION)!
    );
    ...
    if (createdData) setCreated(createdData);
  }, []);
  useEffect(() => {
    if (tawjihiType)
      localStorage.setItem(
        LOCAL_STORAGE_KEY_TAWIJIHI_TYPE,
        JSON.stringify(tawjihiType)
      );
    if (gpa) {
    ...
    if (majorsChoice) {
      localStorage.setItem(
        LOCAL_STORAGE_KEY_MAJORS,
        JSON.stringify(majorsChoice)
      );
    }
    localStorage.setItem(
      LOCAL_STORAGE_KEY_MAJORS_SHOW,
      JSON.stringify(showMajors)
    );
    ...
    localStorage.setItem(
      LOCAL_STORAGE_KEY_PREFRENCES_SELECTED,
      JSON.stringify(prefenceSelected)
    );
    if (name)
  ...
  function handleCaseCreation() {
    const createCase = async () => {
      ...
      if (!idData) {
        setShowToast(true);
        setToastMessage(
          "الاي دي محذوف من وحدة التخزين،يرجى إعادة تنزيل التطبيق"
        );
        return;
      }
      try {
        ...
        if (res.status !== 200) {
          ...
          localStorage.setItem(
            LOCAL_STORAGE_KEY_CASSES_ID,
            JSON.stringify(res.data[1]["caseId"])
          );
  ...
  const setMajorState = (id: string) => {
    ...
    if (majorsChoice.length > 2) {
      ...
      for (var i = 0; i < majorsChoice.length && i < 3; i++) {
        ...
        if (found) currentSelectedMajors.push(found);
      }
      ...
      for (var j = 0; j < currentSelectedMajors.length; j++) {
        if (currentSelectedMajors[j].tags[0] === found!.tags[0]) canAdd = true;
      }
      ...
      //   break;
      // }
      if (canAdd) setMajorChoice([...majorsChoice, id]);
    ...
    if (!tawjihiType) {
      ...
      const majors = majorList.filter((major) => {
        if (major.tawjihiTypes.indexOf(tawjihiType) !== -1) {
          if (parseFloat(gpa) === 0) return major;
          if (parseFloat(gpa) >= parseFloat(major.acceptanceRate)) return major;
          return null;
        }
        return null;
      });
      conetnt = (
        <IonList dir="rtl" style={greyBackGroundColor}>
          <MajorSearch
            majors={majors}
          ...
          <IonCardContent>
            ...
            <div className={classes.ButtonContainer}>
              <IonButton
                ...
                onClick={handlePrefrencesClicked}
              >
                إستمر
              </IonButton>
            </div>
    ...
    component = (
      <>
        ...
        <IonContent dir="rtl" style={style}>
          <IonInfiniteScroll>{conetnt}</IonInfiniteScroll>
        </IonContent>
        <IonToast
          isOpen={showToast}
